...
int main() {
    ...
    if (result == FALSE) {
        MessageBox(nullptr, L"Cannot get current desktop wallpaper path.", L"Message", MB_ICONINFORMATION);
        return -1;
    }
    ...
    if (result == FALSE) {
        MessageBox(nullptr, L"Cannot change desktop wallpaper.", L"Message", MB_ICONINFORMATION);
        return -1;
    }
}

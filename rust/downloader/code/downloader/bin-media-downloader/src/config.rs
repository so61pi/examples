...
pub fn cmdopt() -> Config {
    ...
    let matches = App::new(env!("CARGO_PKG_NAME"))
        ...
        .arg(
            Arg::with_name("download-dir")
                ...
                .value_name("DIR")
                .takes_value(true)
                .required(config.download_dir.is_empty())
        ...
        .arg(
            Arg::with_name("limit-filesize")
                .long("limit-filesize")
                .value_name("SIZE")
                .takes_value(true)
                .required(config.limit_filesize == 0)
                .help("downloaded file size limit"),
        )
        .arg(
    ...
    config.download_dir = get_string(&matches, "download-dir", config.download_dir);
    ...
    config.limit_filesize = get_value(&matches, "limit-filesize", config.limit_filesize);

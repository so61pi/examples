...
auto levenshtein_distance_edit_path(Matrix const& matrix, std::size_t rows, std::size_t cols) -> std::vector<edit> {
    // first = row, second = column
    using pos_type = std::pair<std::size_t, std::size_t>;

    auto min_position = [&](pos_type substitute, pos_type insert, pos_type remove) -> pos_type {
        auto min = substitute;
...
// return edits path
template<typename RandomIt1, typename RandomIt2, typename EqualComparator>
auto levenshtein_distance_path(
    RandomIt1 src_first, RandomIt1 src_last,
    RandomIt2 dst_first, RandomIt2 dst_last,
    EqualComparator ecomp
)
{
    auto matrix = levenshtein_distance_impl(src_first, src_last, dst_first, dst_last, ecomp);
...
{
    return levenshtein_distance_path(src_first, src_last, dst_first, dst_last, std::equal_to<>{});
}
...
int main() {
    ...
    std::cout
        << "Number of edits: "
        << levenshtein_distance(cbegin(str_src), cend(str_src), cbegin(str_dst), cend(str_dst))
        << '\n';

    auto path = levenshtein_distance_path(cbegin(str_src), cend(str_src), cbegin(str_dst), cend(str_dst));
    std::cout
        << "Number of edits: "
        << std::count_if(cbegin(path), cend(path), [](edit val) { return val != edit::nothing; })
        << '\n';
    ...
    std::cout
        << "Number of edits: "
        << levenshtein_distance(cbegin(dt_src), cend(dt_src), cbegin(dt_dst), cend(dt_dst))
        << '\n';
}

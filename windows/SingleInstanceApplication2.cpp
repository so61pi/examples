    ...
    _In_ int nCmdShow)
{
    ...
    // if yes, show that window, then exit
    //
    auto another = FindWindow(szWindowClass, szWindowTitle);
    if (another) {
        MessageBox(
            nullptr,
            L"Another instance of this program is running.",
            L"Message",
            MB_ICONEXCLAMATION);

        ShowWindow(another, SW_NORMAL);
        return 0;
    }
